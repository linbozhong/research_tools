...
MINUTE_1_DB_NAME = 'VnTrader_1Min_Db'
...
class BacktestingEngine(object):
    ...
    def loadData(self):
        ...
        mc = MongoClient()
        # db = mc[DAILY_DB_NAME]
        # db = mc[MINUTE_1_DB_NAME]
        db = mc[MINUTE_30_DB_NAME]
    ...
    def showResult(self):
        ...
        plt.hist(timeseries['netPnl'], bins=100)

        file_path = os.path.join(os.getcwd(), 'result', '{}.png'.format(self.filename))
    ...
    def get_trade(self, symbol=''):
        trade_list = []
        for dt, l in self.tradeDict.items():
            for trade in l:
                if not symbol:
                    trade_list.append((dt, trade))
                elif trade.vtSymbol == symbol:
                    trade_list.append((dt, trade))
        return trade_list
...
class DailyResult(object):
    ...
    def calculateHoldingPnl(self):
        ...
        for vtSymbol, pos in self.posDict.items():
            ...
            # mod.针对不同品种有不同时间周期，有些没有夜盘的品种在夜盘的bar会找不到close价格
            try:
                close = self.closeDict[vtSymbol]
            except KeyError:
                close = previousClose
