...
MAX_PRODUCT_POS = 1         # 单品种最大持仓
MAX_DIRECTION_POS = 60      # 单方向最大持仓
...
class TurtleSignal(object):
    ...
    def calculateIndicator(self):
        ...
        if not self.unit:
            ...
            cstAtr = self.am.atr(self.atrWindow, True)
            print(self.bar.date, self.bar.time)
            print(cstAtr)
...
class TurtlePortfolio(object):
    ...
    def newSignal(self, signal, direction, offset, price, volume):
        ...
        if not unit:
            print('symbol:%s atr:%s' % (signal.vtSymbol, signal.atrVolatility))
            size = self.sizeDict[signal.vtSymbol]
            riskValue = self.portfolioValue * 0.001
