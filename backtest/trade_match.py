...
def generate_trade_df(trades, size, rate, slippage, capital):
    ...
    trade_df["pnl"] = (df["acum_trade_result"] - df["acum_trade_result"].shift(1).fillna(0)) * size

    trade_df["volume"] = df["acum_trade_volume"] - df["acum_trade_volume"].shift(1).fillna(0)
    trade_df["duration"] = df["current_time"] - df["last_time"]
    trade_df["turnover"] = (df["acum_trade_amount"] - df["acum_trade_amount"].shift(1).fillna(0)) * size
